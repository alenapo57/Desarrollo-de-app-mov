...
export default function RegisterScreen({ navigation, onLogin }) {
  ...
  const handleRegister = async () => {
    setError('');
    const { valid, error: validationError } = validateRegisterForm({ nombre, email, password, confirmPassword });
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          ...
          <CustomInput label="Nombre Completo" icon="person" placeholder="Juan Pérez" value={nombre} onChangeText={setNombre} autoCapitalize="words" />
          ...
          <CustomInput label="Contraseña" icon="lock" placeholder="Mínimo 6 caracteres" value={password} onChangeText={setPassword} secureTextEntry />
          <CustomInput label="Confirmar Contraseña" icon="lock-outline" placeholder="Repetí tu contraseña" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry />
          <CustomButton title="Crear Cuenta" onPress={handleRegister} loading={loading} style={styles.registerButton} />
        </View>
